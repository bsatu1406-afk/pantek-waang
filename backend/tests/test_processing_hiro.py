...
def test_dealer_sign_is_inverted_from_customer():
    """A customer BUY of 1 call @ $1.00 (size=10) means the dealer sold 10
    contracts at $1.00 each. Dealer signed premium is -10*100*1.00 = -1000."""
...
def test_calls_and_puts_separated():
    df = pd.DataFrame([
        _trade("2026-01-02T14:30:00", side=1, size=10, price=1.00, opt="C"),  # -1000 dealer call
        _trade("2026-01-02T14:30:01", side=-1, size=5, price=2.00, opt="P"), # +1000 dealer put
    ])
    out = compute_hiro(df, bucket="1min")
    assert out.series[0]["call_premium"] == -1000.0
    assert out.series[0]["put_premium"] == 1000.0
    assert out.series[0]["net_premium"] == 0.0
